...
class LocalFasttextModel(object):
    ...
    def __init__(self, env, config, corpus):
        ...
        print("corpus", corpus)
        self.model = FastText(vector_size=config['dim'], window=config['window'], min_count=1, batch_words=config['batch_words'])
        self.model.build_vocab(corpus_iterable=corpus)
        self.model.train(corpus_iterable=corpus, total_examples=len(corpus), epochs=config['epochs'])
        self.dim = config['dim']
